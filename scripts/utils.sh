...
select_node_type() {
  ...
  node_types="rpc-node collator-node boot-node"
...
set_up_pool_kbytes() {
  ...
  if [ "${pool_kbytes_answer}" = "yes" ]; then
    ...
    echo "PARA_CONF_POOL_KBYTES=${pool_kbytes_answer}" >> "${ENV_FILE}" \
...
public_addr_is_configured() {
  ...
  line="$(grep -E '^[[:space:]]*PARA_CONF_PUBLIC_ADDR=' "${ENV_FILE}" | tail -1 || true)"

  # With no active line, a commented one counts as a deliberate decline
  if [ -z "${line}" ]; then
    if grep -qE '^#+[[:space:]]*PARA_CONF_PUBLIC_ADDR=' "${ENV_FILE}"; then
      return 0
    fi
    return 1
  fi
...
set_public_addr_value() {
  ...
  if [ -z "${value}" ]; then
    line="#PARA_CONF_PUBLIC_ADDR=\"\""
  else
    line="PARA_CONF_PUBLIC_ADDR=\"${value}\""
  fi

  if grep -qE "^#*[[:space:]]*PARA_CONF_PUBLIC_ADDR=" "${ENV_FILE}"; then
    sed -i "s|^#*[[:space:]]*PARA_CONF_PUBLIC_ADDR=.*|${line}|" "${ENV_FILE}" \
      || fn_die "\nError: could not set PARA_CONF_PUBLIC_ADDR in ${ENV_FILE}. Fix it before proceeding. Exiting...\n"
    return
  fi
  ...
  if [ -r "${ENV_FILE_TEMPLATE:-}" ]; then
    anchor="$(awk -F= '/^PARA_CONF_PUBLIC_ADDR=/ { f = 1; next } f && /^[A-Za-z_][A-Za-z0-9_]*=/ { print $1; exit }' "${ENV_FILE_TEMPLATE}")"
  fi

  if [ -n "${anchor}" ] && grep -q "^${anchor}=" "${ENV_FILE}"; then
    sed -i "/^${anchor}=/i ${line}" "${ENV_FILE}" \
      || fn_die "\nError: could not add PARA_CONF_PUBLIC_ADDR to ${ENV_FILE}. Fix it before proceeding. Exiting...\n"
  else
    echo -e "\n${line}" >>"${ENV_FILE}" \
      || fn_die "\nError: could not add PARA_CONF_PUBLIC_ADDR to ${ENV_FILE}. Fix it before proceeding. Exiting...\n"
  fi
...
# Report the outcome once, since both the decline and the empty-list path reach it
...
set_up_public_addr() {
  ...
  sed -i -E "/^PARA_CONF_PUBLIC_ADDR=(\"\"|'')?[[:space:]]*$/d" "${ENV_FILE}" \
    || fn_die "\nError: could not update PARA_CONF_PUBLIC_ADDR in ${ENV_FILE}. Fix it before proceeding. Exiting...\n"
